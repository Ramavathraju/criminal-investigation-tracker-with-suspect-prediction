...
def TrackImages():
    check_haarcascadefile()
    assure_path_exists("Criminalrecorded/")
    assure_path_exists("CriminalDetails/")
    ...
    while True:
        ...
        cv2.imshow('Taking Criminalrecorded', im)
        if (cv2.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("Criminalrecorded\Attendance_" + date + ".csv")
    if exists:
        with open("Criminalrecorded\Attendance_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
        ...
        with open("Criminalrecorded\Attendance_" + date + ".csv", 'a+') as csvFile1:
            ...
            writer.writerow(attendance)
        csvFile1.close()
    with open("Criminalrecorded\Attendance_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
window.title("Criminalrecorded System")
...
message3 = tk.Label(window, text="criminal investigation tracker with suspect prediction" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
lbl3 = tk.Label(frame1, text="Criminalrecorded",width=20  ,fg="black"  ,bg="#00aeff"  ,height=1 ,font=('times', 17, ' bold '))
...
trackImg = tk.Button(frame1, text="Take Criminalrecorded", command=TrackImages  ,fg="black"  ,bg="yellow"  ,width=35  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
